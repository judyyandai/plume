...
class Experiment:
    def __init__(self, vacuum_meter, data_manager, pg, teensy, osc_TDS2014C, osc_DPO2024B, cam, uno, coherent, visa_lock):
        """
        DESCRIPTION:
            Handles logic for running an experiment.
        PARAMETERS:
            vacuum_meter, data_manager, pg, teensy, osc_TDS2014C, osc_DPO2024B, cam, uno, coherent - plupy device connections
            visa_lock - for oscilloscope 
        """
    ...
    def start(self, total_measurements, mode, option, folder):
        """
        DESCRIPTION:
            Starts the T_Pressure thread and T_experiment thread.
        PARAMETERS
    ...
    def experiment(self, option, folder, total_measurements = 0, mode = None):
        ...
        while self.e_experimentOn.is_set():
            ...
            self.measure(current_prepulse_mode, image)
    ...
    def invalid_measurement(self):
        """
        DESCRIPTION:
           Calls to update GUI plume text for an invalid measurement.
        PARAMETERS:
        """
    ...
    def valid_measurement(self, mode, current_prepulse_mode, image):
        ...
        PARAMETERS:
            mode - "rs" for run series, "ps" for power series, None for not a series.
    ...
    def measure(self, current_prepulse_mode, image):
        """
        DESCRIPTION:
           Changes camera gain if needed, runs pulse generator, starts tdc, collects tdc data and starts camera thread.
        PARAMETERS:
            current_prepulse_mode - true (prepulse) or false (no prepulse) 
            image - zero array created to copy image buffer into
        """
    ...
    def sort_tdc(self, tdc_counts, tdc_channels):
        ...
        PARAMETERS
            tdc_counts: lists of integers. Each represents time in ns since the start of TDC recording when an electrical signal was received.
    ...
    def camera_control(self, local_image):
        ...
        DESCRIPTION:
            ...
            experimental thread. 
            A callback to experiment controller is given to display the image on the GUI.
        PARAMETERS:
    ...
    def get_pirl_timestamp(self, data):
        """
        DESCRIPTION:
            Return the time in nanoseconds at which the PIRL ablation pulse occurs.
            Because many PIRL pulses are recorded by the TDC, this function returns the relevant one to our process. 
